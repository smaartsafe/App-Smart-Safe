...
const Inicio = () => {
        ...
        () => {
          getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
            ...
            utils
              .fetchUserProfile()
              .then((profile) => {
                const { nome, sobrenome, rua, bairro, cidade, estado } =
                  profile;
  ...
  const handleEmergencyCall = async () => {
    try {
      ...
      const userContactsRef = ref(db, `Contatos/${user.uid}`);
      ...
      let favoritedContactFound = false;
      snapshot.forEach((contactSnapshot) => {
        ...
        if (contact.favorited) {
          ...
          if (userLocation && userLocation.coords) {
            ...
            favoritedContactFound = true;
            return true;
          } else {
            console.error("Localização não disponível");
            Alert.alert(
              "Erro ao enviar a mensagem: Localização não disponível."
            );
          }
        }
      });
      ...
      Alert.alert(
        "Erro ao acionar o contato de emergência. Por favor, tente novamente mais tarde."
      );
    }
  };
    ...
    <View style={styles.container}>
      ...
      {uploaded && (
        <Text style={styles.uploadedText}>
          Pedido de SOS enviado com sucesso!
        </Text>
      )}
      <TouchableOpacity
